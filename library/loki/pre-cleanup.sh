#!/usr/bin/env bash
# Pre-cleanup hook for Loki.
# Runs in the context of the cleanup script with access to:
#   $app_name, $manifest_name, and all functions.

# Remove the Loki data source from Grafana provisioning
grafana_app="$(find_app_by_manifest "grafana" 2>/dev/null || true)"
if [[ -n "$grafana_app" ]]; then
  provisioning_file="/var/lib/dokku/data/storage/${grafana_app}-provisioning/datasources/loki.yml"
  if [[ -f "$provisioning_file" ]]; then
    echo "       Removing Loki data source from Grafana"
