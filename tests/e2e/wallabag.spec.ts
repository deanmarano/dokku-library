...
const APP_NAME = "test-wallabag";

test.describe("library:checkout wallabag (with postgres)", () => {
  test.beforeAll(() => {
    test.skip(!pluginAvailable("postgres"), "postgres plugin not available");

    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout wallabag --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --non-interactive`,
        { timeout: 300_000 }
      );
    }
  });
  ...
  test("should serve wallabag UI", async () => {
    const url = getAppUrl(APP_NAME);
    ...
    expect(body.toLowerCase()).toContain("wallabag");
  });
