...
const APP = "test-sonarr";
const LIBRARY_APP = "sonarr";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can access Sonarr system status page", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    // Sonarr has no app auth - Authelia handles it
    await page.goto(`http://${DOMAIN}/system/status`);
    await page.waitForLoadState("networkidle");
    const content = await page.content();
    expect(
      content.includes("Sonarr") ||
