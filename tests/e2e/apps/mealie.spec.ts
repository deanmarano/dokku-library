...
const APP = "test-mealie";
const LIBRARY_APP = "mealie";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can log into Mealie", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    // Mealie default credentials: changeme@email.com / MyPassword
    const loginForm = page.locator('input[type="email"], input[name="email"]');
    if (await loginForm.isVisible({ timeout: 5000 }).catch(() => false)) {
      await loginForm.fill("changeme@email.com");
    ...
    expect(
      content.includes("Mealie") ||
        content.includes("Recipe") ||
        content.includes("Meal Plan")
    ).toBe(true);
  });
