...
const APP = "test-jellyfin";
const LIBRARY_APP = "jellyfin";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can access Jellyfin setup wizard", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    await page.waitForLoadState("networkidle");
    const content = await page.content();
    // Jellyfin shows setup wizard on first launch
    expect(
      content.includes("Jellyfin") ||
        content.includes("Welcome") ||
        content.includes("setup") ||
        content.includes("Preferred display language")
    ).toBe(true);
  });
