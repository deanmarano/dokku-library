...
const APP = "test-wallabag";
const LIBRARY_APP = "wallabag";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can log into Wallabag", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    // Wallabag default credentials: wallabag/wallabag
    const loginForm = page.locator("#username");
    if (await loginForm.isVisible({ timeout: 5000 }).catch(() => false)) {
      await loginForm.fill("wallabag");
      await page.locator("#password").fill("wallabag");
      await page.locator('button[type="submit"]').click();
      await page.waitForLoadState("networkidle");
    }
    const content = await page.content();
    expect(
      content.includes("wallabag") ||
