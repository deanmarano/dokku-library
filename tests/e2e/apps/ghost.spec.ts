...
const APP = "test-ghost";
const LIBRARY_APP = "ghost";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can access Ghost admin setup", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    await page.goto(`http://${DOMAIN}/ghost/#/setup`);
    await page.waitForLoadState("networkidle");
    // Ghost setup wizard should be visible
