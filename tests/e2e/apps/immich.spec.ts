...
const APP = "test-immich";
const LIBRARY_APP = "immich";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can access Immich registration form", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    await page.waitForLoadState("networkidle");
    const content = await page.content();
    expect(
      content.includes("Immich") ||
        content.includes("Create Account") ||
        content.includes("Admin Registration") ||
