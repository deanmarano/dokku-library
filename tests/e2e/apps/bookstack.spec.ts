...
const APP = "test-bookstack";
const LIBRARY_APP = "bookstack";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("checkout succeeds and app is running", async () => {
    ...
    const httpReady = await waitForHttp(`http://${DOMAIN}/`, 90_000);
  ...
  test("can log into BookStack and create a book", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    // BookStack default credentials: admin@admin.com / password
    const loginForm = page.locator('input[name="email"], #email');
    if (await loginForm.isVisible({ timeout: 5000 }).catch(() => false)) {
      await loginForm.fill("admin@admin.com");
    ...
    expect(
      content.includes("BookStack") ||
        content.includes("Books") ||
        content.includes("Shelves")
    ).toBe(true);
  });
