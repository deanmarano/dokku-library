...
const APP = "test-nextcloud";
const LIBRARY_APP = "nextcloud";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can access Nextcloud login page", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    await page.waitForLoadState("networkidle");
    const adminPassword = getConfig(APP, "NEXTCLOUD_ADMIN_PASSWORD");
    // Nextcloud has its own login
    const loginForm = page.locator('#user');
    if (await loginForm.isVisible({ timeout: 5000 }).catch(() => false)) {
      await loginForm.fill("admin");
      await page.locator('#password').fill(adminPassword);
      await page.locator('button[type="submit"], #submit-form').click();
      await page.waitForLoadState("networkidle");
    }
