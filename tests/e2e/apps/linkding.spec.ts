...
const APP = "test-linkding";
const LIBRARY_APP = "linkding";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can log into Linkding and see bookmarks", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    const adminPassword = getConfig(APP, "LD_SUPERUSER_PASSWORD");
    // Linkding has its own login
