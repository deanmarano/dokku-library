...
const APP = "test-miniflux";
const LIBRARY_APP = "miniflux";
...
test.describe(`${LIBRARY_APP} e2e`, () => {
  ...
  test("can log into Miniflux and create category", async ({ page }) => {
    await page.goto(`http://${DOMAIN}/`);
    await loginViaAuthelia(page, TEST_USER.username, TEST_USER.password);
    const adminPassword = getConfig(APP, "ADMIN_PASSWORD");
    // Miniflux has its own login
    ...
    expect(
      content.includes("Miniflux") ||
        content.includes("Feeds") ||
