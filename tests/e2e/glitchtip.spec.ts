...
const APP_NAME = "test-glitchtip";

test.describe("library:checkout glitchtip (with postgres + redis)", () => {
  test.beforeAll(() => {
    ...
    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout glitchtip --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --no-mail --non-interactive`,
        { timeout: 600_000 }
      );
    }
  });
