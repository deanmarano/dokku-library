...
const APP_NAME = "test-plausible";

test.describe("library:checkout plausible (with postgres)", () => {
  test.beforeAll(() => {
    test.skip(!pluginAvailable("postgres"), "postgres plugin not available");

    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout plausible --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --no-mail --non-interactive`,
        { timeout: 300_000 }
      );
    }
  });
  ...
  test("should serve plausible UI", async () => {
    const url = getAppUrl(APP_NAME);
    ...
    expect(body.toLowerCase()).toContain("plausible");
  });
