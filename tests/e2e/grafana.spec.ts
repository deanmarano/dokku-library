...
const APP_NAME = "test-grafana";

test.describe("library:checkout grafana (with postgres)", () => {
  test.beforeAll(() => {
    test.skip(!pluginAvailable("postgres"), "postgres plugin not available");

    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout grafana --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --non-interactive`,
        { timeout: 300_000 }
      );
    }
  });
  ...
  test("should serve grafana login page", async () => {
    const url = getAppUrl(APP_NAME);
