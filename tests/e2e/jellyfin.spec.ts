...
const APP_NAME = "test-jellyfin";

test.describe("library:checkout jellyfin (no database)", () => {
  test.beforeAll(() => {
    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout jellyfin --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --non-interactive`,
        { timeout: 300_000 }
      );
    }
  });
  ...
  test("should serve jellyfin UI", async () => {
    const url = getAppUrl(APP_NAME);
