...
const APP_NAME = "test-radarr";

test.describe("library:checkout radarr (no database)", () => {
  test.beforeAll(() => {
    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout radarr --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --non-interactive`,
        { timeout: 300_000 }
      );
    }
  });
  ...
  test("should have storage mounts configured", () => {
    ...
    expect(report).toContain("/movies");
  ...
  test("should serve radarr UI", async () => {
    const url = getAppUrl(APP_NAME);
    ...
    expect(body.toLowerCase()).toContain("radarr");
  });
