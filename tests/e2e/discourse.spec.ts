...
const APP_NAME = "test-discourse";

test.describe("library:checkout discourse (with postgres + redis)", () => {
  test.beforeAll(() => {
    ...
    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout discourse --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --no-mail --non-interactive`,
        { timeout: 600_000 }
      );
    }
  });
  ...
  test("should have storage mount configured", () => {
    ...
    expect(report).toContain("/bitnami/discourse/public/uploads");
  });

  test("should be running and healthy", async () => {
    const healthy = await waitForHealthy(APP_NAME, 180_000);
