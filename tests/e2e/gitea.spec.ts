...
const APP_NAME = "test-gitea";

test.describe("library:checkout gitea (with postgres)", () => {
  test.beforeAll(() => {
    test.skip(!pluginAvailable("postgres"), "postgres plugin not available");

    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout gitea --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --non-interactive`,
        { timeout: 300_000 }
      );
    }
  });
  ...
  test("should serve gitea UI", async () => {
    const url = getAppUrl(APP_NAME);
    ...
    expect(body.toLowerCase()).toContain("gitea");
  });
