...
const APP_NAME = "test-mealie";

test.describe("library:checkout mealie (with postgres)", () => {
  test.beforeAll(() => {
    test.skip(!pluginAvailable("postgres"), "postgres plugin not available");

    if (!appExists(APP_NAME)) {
      dokku(
        `library:checkout mealie --name=${APP_NAME} --domain=${APP_NAME}.test.local --no-ssl --no-auth --non-interactive`,
        { timeout: 300_000 }
      );
    }
  });
  ...
  test("should have storage mount configured", () => {
    ...
    expect(report).toContain("/app/data");
  });
  ...
  test("should have healthy API endpoint", async () => {
    const url = getAppUrl(APP_NAME);
    const response = await fetch(`${url}/api/app/about`, {
      signal: AbortSignal.timeout(10_000),
    });
    expect(response.ok).toBe(true);
    const json = await response.json();
    expect(json.production).toBe(true);
  });

  test("should serve mealie UI", async () => {
    const url = getAppUrl(APP_NAME);
    ...
    expect(body.toLowerCase()).toContain("mealie");
  });
